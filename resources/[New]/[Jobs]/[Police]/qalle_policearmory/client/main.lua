	...
	while true do
		...
		if not Config.OnlyPolicemen or (Config.OnlyPolicemen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police") then
			...
			if dstCheck <= 5.0 then
				...
				local text = "Armurerie LSPD"

				if dstCheck <= 0.5 then
					text = "[~g~E~s~] Armurerie LSPD"
...
OpenPoliceArmory = function()
	...
	local elements = {
		{ ["label"] = "Prendre une Armes", ["action"] = "weapon_storage" },
		{ ["label"] = "Déposé une Armes", ["action"] = "weapon_supp" }
	}
	...
	function(data, menu)
		...
		elseif action == "weapon_supp" then
			...
			ESX.ShowNotification("Vous avez déposé vos armes de service.")
		end		
