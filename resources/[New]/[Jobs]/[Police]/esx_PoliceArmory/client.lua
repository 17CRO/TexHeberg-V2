	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_policeArmory_kevlar_menu",
			{
				...
				align    = "top-left",
				elements = {
					{label = Config.Vest1, armor = 25}, --16
					{label = Config.Vest2, armor = 100}, --10
					{label = Config.Vest3, armor = 125}, --7
					{label = Config.Vest4, armor = 75}, --4
					{label = Config.Vest5, armor = 85}, --4
		...
		function(data, menu)
			...
			if data.current.armor == 0 then
				...
				elseif data.current.armor == 125 then
				...
				elseif data.current.armor == 85 then
...
function ListOfAttachments(type,name,weaponhash,attachment,flashlight,scope,suppressor)
	...
	if flashlight then
		...
		if state then
			text = "Lampe Torche: "..('<span style="color:green;">%s</span>'):format("Installer")
		else
			text = "Lampe Torche: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
		end
	...
	if scope then
		...
		if state then
			text = "Viseur: "..('<span style="color:green;">%s</span>'):format("Installer")
		else
			text = "Viseur: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
		end
		...
		if state then
			text = "Poigné: "..('<span style="color:green;">%s</span>'):format("Installer")
		else
			text = "Poigné: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
		end
	...
	if suppressor then
		...
		if state then
			text = "Silencieux: "..('<span style="color:green;">%s</span>'):format("Installer")
		else
			text = "Silencieux: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'policeArmory_list_of_attachments', {
		...
		if data.current.value == flashlight then
			if data.current.state then
				newData.label = "Lampe Torche: "..('<span style="color:green;">%s</span>'):format("Installer")
				...
				newData.label = "Lampe Torche: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
		...
		elseif data.current.value == scope then
			if data.current.state then
				newData.label = "Viseur: "..('<span style="color:green;">%s</span>'):format("Installer")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeEquipped,name))
			else
				newData.label = "Viseur: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeRemove,name))
			end
		elseif data.current.value == grip then
			if data.current.state then
				newData.label = "Poigné: "..('<span style="color:green;">%s</span>'):format("Installer")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.GripEquipped,name))
			else
				newData.label = "Poigné: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.GripRemove,name))
			end
		elseif data.current.value == suppressor then
			if data.current.state then
				newData.label = "Silencieux: "..('<span style="color:green;">%s</span>'):format("Installer")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.SuppressorEquipped,name))
			else
				newData.label = "Silencieux: "..('<span style="color:red;">%s</span>'):format("Désinstaller")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
