...
AddEventHandler('bank:deposit', function(amount)
	...
	if amount == nil or amount <= 0 or amount > xPlayer.getMoney() then
		...
		TriggerClientEvent('esx:showAdvancedNotification', _source, 'Maze Bank', 'Account Notification', 'Montant invalid.', 'CHAR_BANK_MAZE', 9)
	else
		xPlayer.removeMoney(amount)
		xPlayer.addAccountMoney('bank', tonumber(amount))
                -- advanced notification with bank icon
		TriggerClientEvent('esx:showAdvancedNotification', _source, 'Maze Bank', 'Account Notification', 'Vous avez déposer: ~g~$' .. amount .. '~s~', 'CHAR_BANK_MAZE', 9)
	end
end)
...
AddEventHandler('bank:withdraw', function(amount)
	...
	if amount == nil or amount <= 0 or amount > base then
		...
		xPlayer.addMoney(amount)
                 -- advanced notification with bank icon
                TriggerClientEvent('esx:showAdvancedNotification', _source, 'Maze Bank', 'Account Notification', 'Vous avez retirer: ~r~$' .. amount .. '~s~', 'CHAR_BANK_MAZE', 9)
	end
end)
