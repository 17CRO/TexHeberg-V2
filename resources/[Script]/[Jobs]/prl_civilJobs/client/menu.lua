RMenu.Add('LifeInvaders', 'main', RageUI.CreateMenu("LifeInvaders", " "))
RMenu:Get('LifeInvaders', 'main'):SetSubtitle("~b~LifeInvaders central d'intérim")
            ...
            for _,v in pairs(metier) do
                RageUI.Button(v.nom, v.desc, {}, true, function(Hovered, Active, Selected)
                    if (Selected) then
                        ...
                        DestroyCam(cam, 1)
                        RageUI.Popup({
                            message = "Vous avez choisis de ~b~"..v.nom.."~w~ ? Très bien, je vous ai donné les coordonées GPS sur votre téléphone. ~g~Merci d'utiliser les services LifeInvaders !",
                            sound = {
